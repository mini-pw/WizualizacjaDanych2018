...
kraj<-c("Strefa euro", "Niemcy", "W.Brytania", "Francja", "Hiszpania", "W³ochy")
...
p1<-ggplot(data, aes(x=indeks, y=wartosc_notowan, fill=pelna_nazwa))+
  geom_bar(stat="identity")+labs(x="Indeks", y="Wartoœæ notowañ (pkt.)", fill="kraj-indeks")
p1<-p1+scale_fill_brewer(palette="Dark2") +ggtitle("Wartoœæ notowañ na dzieñ 13.02.2019 godz. 9:40")
p1
#dane dodatkowe (liczba ludnoœci)
...
geom_point()+scale_size_continuous(range = c(4, 12))+ ylim (2000, 21000) +
  labs(x='Indeks', y='Wartoœæ notowañ (pkt.)', colour="kraj-indeks",size="populacja kraju")
p2<-p2+scale_colour_brewer(palette="Dark2")+ggtitle("Wartoœæ notowañ uwzglêdniaj¹c liczebnoœæ populacji odpowiadaj¹cym im krajów")
p2

p3<-ggplot(data, aes(x=indeks, y=przyrost_wartosci, fill=pelna_nazwa, label=paste0(przyrost_procentowy,"%")))+
  geom_bar(stat="identity")+labs(x = "Indeks", y="Przyrost wartoœci notowañ (pkt.)")+geom_text(vjust =- 1, size = 4)+
  ylim(0,110)
p3<-p3+scale_fill_brewer(palette="Dark2")+ggtitle("Wzrost wartoœci notowañ w stosunku do dnia poprzedniego")
p3
