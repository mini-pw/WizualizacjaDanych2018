...
football_data_team_view %>% 
  plot_goals_over_time(selected_team = "Arsenal",
                       selected_opponent = "Liverpool")
...
server <- function(input, output, session) {
  ...
  output$goals_over_time_plot <- renderPlot({
    ...
    football_data_team_view %>% 
      plot_goals_over_time(selected_team = input$selected_team,
  ...
  output$win_amount_box <- renderValueBox({
    results <- football_data_team_view %>% 
      calculate_win_loss_amounts(input$selected_team,
                                 input$selected_opponent)
