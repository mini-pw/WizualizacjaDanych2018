...
output:
  html_document:
    df_print: paged
---
Zestawienie konsumpcji jedynie jako koloru wydawało mi się nieczytelne.
Mój pierwotny zamysł polegał na przedstawieniu konsumpcji w formie słupków i ceny jako lini na jednym wykresie, ale 
sam autor ggplota odwiódł mnie od tego pomysłu (https://stackoverflow.com/a/3101876/5599570 ).
Ostatecznie zdecydowałem się podzielić wykres na dwa.
...
Moja wersja
```{r message=FALSE, warning=FALSE}
consumption <- sugar_in_Poland %>% 
  ggplot(aes(x = years)) +
  ggtitle('Sugar consumption in Poland (2010-2017)') +
  ylab('Consumption pp (kg)') +
  geom_bar(aes(y = sugar_consumption_per_capita), stat = 'identity') +
  theme_bw()

prices <- sugar_in_Poland %>%
  ggplot(aes(x = years)) +
  scale_y_continuous(expand = c(0,0), limits = c (0,5)) +
  ylab('Sugar price per kg (PLN)') +
  geom_line(aes(y = sugar_price))
